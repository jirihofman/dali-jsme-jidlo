 ...
 *  Under MIT License
 * 
 *  Modified by Jiri Hofman to show count and total price. Heat is calculated by total price.
 */
( function( $ ) {
    ...
    "use strict";
        ...
        $.extend( Plugin.prototype, {
            ...
            _calculateBins: function( events ) {
                ...
                for ( i in events ) {
                    events[ i ].level = this._matchBin( binlabelrange, events[ i ].total );
                }
            ...
            calendarHeatmap: function( ) {
                ...
                for ( i = 0; i < blocks.length; i++ ) {
                    ...
                    for ( var j = 0; j < days; j++ ) {
                        ...
                        if ( obj ) {
                            let ordersWording = 'objednávek';
                            if (obj.count === 1) {
                                ordersWording = 'objednávka';
                            } else if (obj.count <= 4) {
                                ordersWording = 'objednávky';
                            }
                            var title = [obj.count, ordersWording, 'za', (obj.total || 0), 'Kč dne', this._dateFormat(obj.date, "dddd DD. MMMM, YYYY")].join(' ');
                            var color = "";
